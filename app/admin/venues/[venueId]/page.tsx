...
export default async function AdminVenuePage({
  ...
  return (
    <AdminShell
      badge="Заведение"
      title={venue.name}
      description="Короткий обзор денег, заявок и текущей очереди. Всё управление вынесено в настройки."
    ...
    >
      ...
      <div className="grid gap-5 md:grid-cols-4">
        <SectionCard className="md:col-span-2">
          <div className="flex items-start justify-between gap-4">
            <div>
              ...
              <div className="mt-2 text-4xl font-semibold text-gradient">{formatPrice(analytics.balanceCents)}</div>
        ...
        <SectionCard>
          ...
          <div className="mt-2 text-3xl font-semibold">{formatPrice(grossCents)}</div>
            ...
            <div>
              ...
              <div className="mt-4 space-y-3">
                ...
                <div className="rounded-[1.2rem] border border-white/10 bg-white/[0.03] p-4">
                  ...
                  <div className="mt-1 font-semibold">{formatPrice(venue.requestPriceCents)}</div>
                </div>
                <div className="rounded-[1.2rem] border border-white/10 bg-white/[0.03] p-4">
                  <div className="text-sm text-white/45">Разрешено треков</div>
                  <div className="mt-1 font-semibold">{venue.venueTracks.length}</div>
                </div>
              </div>
            </div>
            <Link
              href={`${baseHref}/settings` as Route}
              className="inline-flex justify-center rounded-full bg-gradient-to-r from-accent to-accentBlue px-5 py-3 text-sm font-semibold text-white shadow-glow"
            >
              Перейти в настройки
            </Link>
          </div>
      ...
      <SectionCard>
        ...
        {queuedItems.length === 0 ? (
          <div className="mt-5 rounded-[1.4rem] border border-dashed border-white/10 p-5 text-sm text-white/45">
          ...
          <div className="mt-5 overflow-hidden rounded-[1.5rem] border border-white/10">
            ...
            <div className="divide-y divide-white/10">
              {queuedItems.map((item) => (
                <div
                  key={item.id}
                  className="grid grid-cols-1 gap-3 px-4 py-4 text-sm text-white/75 md:grid-cols-[1.4fr_0.7fr_0.9fr_1fr]"
                >
                  <div>
                  ...
                  <div>
                    ...
                    <div className="mt-2 text-white/45">#{item.position}</div>
                  </div>
