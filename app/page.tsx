      ...
      <div className="mx-auto grid min-h-[calc(100vh-5rem)] max-w-6xl gap-8 lg:grid-cols-[1.05fr_0.95fr] lg:items-center">
        <section className="space-y-8">
          <div className="inline-flex rounded-full border border-white/10 bg-white/5 px-4 py-2 text-xs uppercase tracking-[0.35em] text-white/60">
            QueueBeat for venues
          </div>
          <div className="space-y-4">
            <h1 className="max-w-2xl text-5xl font-semibold leading-tight sm:text-6xl">
              <span className="text-gradient">Launch your QR music requests</span>
              <br />
              in one dashboard
            </h1>
            <p className="max-w-xl text-lg leading-8 text-white/70">
              Register your venue, pass verification, build an approved playlist, download the QR code, and let
              guests request paid tracks from their phones.
            </p>
          </div>
        ...
        <SectionCard className="space-y-5">
          <div>
            <div className="text-xs uppercase tracking-[0.35em] text-white/35">How it works</div>
            <h2 className="mt-3 text-2xl font-semibold">One service, three roles</h2>
          </div>
          {[
            ["Service owner", "Approves venues and sees every client account in /platform."],
            ["Venue account", "Manages price, playlist, QR code, queue, and request availability."],
            ["Guest", "Scans QR and lands on the exact venue playlist at /v/[slug]."]
